...
def generate_recommendations(url: str, title: str, text: str) -> Dict:
    ...
    if not openai.api_key:
        return {
            "summary": "API 키가 설정되지 않아 추천을 생성할 수 없습니다.",
            "recommendations": []
        }
...
1. 주제(대상)에 대한 50~100자 내외의 요약 설명 (summary 라는 키값을 갖도록)
2. 주제와 관련되어 사용자가 관심있어 할 것이라고 생각되는 컨텐츠 3가지 추천: 컨텐츠는 url로 접근 가능하여야 하며 해당 사이트(컨텐츠)에 대한 간략한 설명과 url 링크를 포함한다.(recommend 라는 키값을 갖도록)
...
출력은 위와 같이 바로 사용 가능한 JSON 형식이어야 하며 해당 JSON외엔 그 어떤 내용도 답변에 포함되지 않도록 해야한다.
"""},
        ...
        try:
            ...
            # 추천 항목들을 리스트로 변환
            recommendations = []
            for i in range(1, 4):  # recommend1, recommend2, recommend3
                key = f"recommend{i}"
                if key in result:
                    recommendations.append(result[key])
